...
//            getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
    ...
    public ArrayList<Double> getDetectionData(){
        ...
        if(serialPortOpe.initDevice()) {
            ...
            for (int i=0;i<data.length-CURRENT_CHANEEL*2;i++){
                if (data[i]==0x08&&data[i+1]==0x5A&&data[i+CURRENT_CHANEEL*2]==0x08&&data[i+CURRENT_CHANEEL*2+1]==0x5A){
                    i+=(CURRENT_CHANEEL*2-1);//如果满足 i跳到下一个标记
                    int nChannelNumber= SystemParameter.getInstance().nChannelNumber;
                    for (int j=0;j<nChannelNumber+2;j++){
                        list.add(NumberHelper.bytesToInt(new byte[]{data[i+2+j*2],data[i+3+j*2]},0));    //低位在前转换
                    }
                }
            }
            serialPortOpe.stopReciveData();//获取一组数据后，立即停止接收数据，关闭设备
            resultList =dataProcess(list);//从设备获取数据
        }else{
    ...
    public ArrayList<Double> dataProcess(List<Integer> data){
        int count = 0;
        int nChannelNumber =SystemParameter.getInstance().nChannelNumber;
        ...
        for (int dataIndex = 0; dataIndex < data.size()/nChannelNumber; dataIndex++) {
            if(count<20) {//取20组作平均
                for (int i = 0; i < nChannelNumber; i++) {
                    nTempList.get(i).add(Double.valueOf(data.get(dataIndex+i+2)));
                }
                count++;
            }else{
                break;
